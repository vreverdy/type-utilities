...
\newcommand{\presfile}{p1016r0_presentation.pdf}
...
%% -------------------------------------------------------------------------- %%
\rSec1[proposal.introduction]{Introduction}
...
\begin{itemize}
\item pointers removal: \tcode{remove_all_pointers} inspired from \tcode{remove_all_extents}
\item qualifiers manipulation: \tcode{copy_*} and \tcode{clone_*} type traits
\item inheritance: \tcode{empty_base} struct helper, \tcode{is_instantiable}, \tcode{is_inheritable} and \tcode{inherit_if}
\item callables categorization: \tcode{is_closure}, \tcode{is_functor}, \tcode{is_function_pointer}, \tcode{is_function_object} and \tcode{is_callable}
\item metaprogramming helpers: \tcode{index_constant}, \tcode{type_t}, \tcode{false_v} and \tcode{true_v}
\end{itemize}
...
%% -------------------------------------------------------------------------- %%
\rSec2[proposal.design.removal]{Pointers removal}
...
%% -------------------------------------------------------------------------- %%
\rSec2[proposal.design.inher]{Inheritance}
...
\begin{itemize}
\item \tcode{empty_base}: an generic templated empty class
\item \tcode{is_instantiable}: a trait to detect whether an object from the type can be instantiated
...
\item keep \tcode{monostate} specific to \tcode{variant} and introduce a new universally accepted empty class provided by the standard library such as \tcode{blank} (the name being inspired from \href{https://www.boost.org/doc/libs/1_66_0/boost/blank.hpp}{The Boost C++ Libraries}), and introduce a templated version
\item introduce a different empty class for each specific use case such as \tcode{empty_base}
\end{itemize} 
%% -------------------------------------------------------------------------- %%
\rSec2[proposal.design.call]{Callables categorization}
...
\item \tcode{empty_base}
\item \tcode{is_instantiable}
...
\begin{itemize}
\item \tcode{is_instantiable}: what should be the exact definition of that trait?
...
\indexlibrary{\idxcode{empty_base}}

\pnum
The class template \tcode{empty_base} provides a templated empty class to be used in a wide range of context such as being a placeholder in conditional inheritance.
...
\indexlibrary{\idxcode{is_instantiable}}%
\tcode{template<class T>\br struct is_instantiable;} &
It is possible to instantiate an object of type \tcode{T}. &
\\ \rowsep
...
The member typedef \tcode{type} names the same type as \tcode{conditional_t<b, T, empty_base<T>>>}.
